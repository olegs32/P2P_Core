...
from src.proxy import ProxyDomain
...
domain = ProxyDomain(DOMAIN, hostname, router)
...
async def route(src: str, dst: str, data: Request):
    ...
    response = await router.route(src, dst, data)
    print(response)
    return response
    # if response.get('stat', {}).get('service'):
    #     # REQUEST_TRACKER[src][response['id']] = dst
    #     return {'success': True, "status": "Message delivered"}
    # else:
    #     return {'success': False, "status": "Failed to deliver", "error": response.get("data")}
