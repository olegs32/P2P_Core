...
"""
from layers.service import MethodRegistry
import asyncio
import logging
import threading
import time
...
class P2PApplicationContext:
    ...
    def register_method(self, path: str, method: callable) -> None:
        ...
        self._method_registry.register(path, method)
    ...
    def unregister_method(self, path: str) -> bool:
        ...
        return self._method_registry.unregister(path)

    @property
    def method_registry(self) -> Dict[str, Any]:
        """Безопасный доступ к реестру методов (только чтение)"""
