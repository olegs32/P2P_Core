...
- `--clean` - Удалить build/ и dist/ перед сборкой (рекомендуется)
- `--onedir` - Создать папку с файлами вместо одного .exe (быстрее сборка)
...
### Data файлы
- `dist/services/` - все сервисы со всем содержимым
...
Убедитесь что `dist/services/` существует и содержит сервисы. При `--clean` сервисы сохраняются автоматически.
