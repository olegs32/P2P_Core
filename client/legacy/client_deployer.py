...
class Client:
    ...
    async def auth(self):
        async with httpx.AsyncClient() as client:
            resp = await client.get(f'http://{self.SERVER}/register', params={
                'id': self.id,
                'hostname': self.hostname,
                'ts': time.time()
            })
            resp_data = resp.json()
            self.id = resp_data['id']
            return resp_data

    async def get_config(self):
        async with httpx.AsyncClient() as client:
            resp = await client.get(f'http://{self.SERVER}/cicd/get_config', params={
                'id': self.id,
    ...
    def ping(self):
        resp = requests.post(f'http://{self.SERVER}/ping?id={self.id}&ts={time.time()}', json=self.services).json()
        if resp['status'] == 409:
            print(self.register())
        else:
...
cli = Client()
dpinger_thread = Thread(target=cli.dpinger, daemon=True)
dpinger_thread.start()

while True:
    time.sleep(1)
    if not dpinger_thread.is_alive():
        dpinger_thread = Thread(target=cli.dpinger, daemon=True)
        dpinger_thread.start()
